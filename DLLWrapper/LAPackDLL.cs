...
namespace PavelStransky.DLLWrapper {
    ...
    /// </summary>
    unsafe public class LAPackDLLLoader {
        /// <summary>
        /// 32-bitová verze
        /// </summary>
    ...
    unsafe public class LAPackDLL {
        ...
        public static Vector[] dsyev(Matrix matrix, bool ev) {
            ...
            try {
                ...
                LAPackDLLLoader.dsyev(&jobz, &uplo, &n, a, &lda, w, &lwork1, &lwork, &info);

                lwork = (int)lwork1;
                work = Memory.NewDouble(lwork);

                // Vlastní výpoèet
                LAPackDLLLoader.dsyev(&jobz, &uplo, &n, a, &lda, w, work, &lwork, &info);
                success = true;
            }
            finally {
                Memory.Delete(work);
        ...
        public static Vector[] dsbevx(SymmetricBandMatrix band, bool ev, int iMin, int iMax) {
            ...
            bool success = false;
            try {
                LAPackDLLLoader.dsbevx(&jobz, &range, &uplo, &n, &kd, ab, &ldab, q, &ldq,
