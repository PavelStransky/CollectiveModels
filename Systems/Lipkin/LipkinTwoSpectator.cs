...
namespace PavelStransky.Systems {
    public class LipkinTwoSpectator: LipkinFull, IQuantumSystem, IEntanglement {
        private double alphaS, omegaS, g;

        /// <summary>
        /// Prázdný konstruktor
        /// </summary>
        protected LipkinTwoSpectator() { }

        /// <summary>
        /// Konstruktor
        /// </summary>
        public LipkinTwoSpectator(double alpha, double omega, double alphaS, double omegaS, double g)
            : base(alpha, omega) {
            this.alphaS = alphaS;
            this.omegaS = omegaS;
            this.g = g;
        ...
        public override void HamiltonianMatrix(IMatrix matrix, BasisIndex basisIndex, IOutputWriter writer) {
            ...
            double kS = n2 == 0 ? 0.0 : -(1.0 - this.alphaS) / n2;
            ...
            for(int i = 0; i < dim; i++) {
                ...
                // 1 + 5
                matrix[i, i] = this.alpha * c1 + this.alphaS * c1S + k * this.omega * this.omega * c1 * c1 + kS * this.omegaS * this.omegaS * c1S * c1S;
                ...
                if((j = index[m1, m2 + 4]) >= 0)
                    matrix[j, i] = kS * this.ShiftPlus(l2, m2) * this.ShiftPlus(l2, m2 + 2);
                if((j = index[m1, m2 + 2]) >= 0) {
                    matrix[i, i] += kS * this.ShiftPlus(l2, m2) * this.ShiftMinus(l2, m2 + 2);
                    matrix[j, i] = kS * this.omegaS * this.ShiftPlus(l2, m2) * (n2 + m2 + 1);
                }
                // 3 S-
                if((j = index[m1, m2 - 4]) >= 0)
                    matrix[j, i] = kS * this.ShiftMinus(l2, m2) * this.ShiftMinus(l2, m2 - 2);
                if((j = index[m1, m2 - 2]) >= 0) {
                    matrix[i, i] += kS * this.ShiftMinus(l2, m2) * this.ShiftPlus(l2, m2 - 2);
                    matrix[j, i] = kS * this.omegaS * this.ShiftMinus(l2, m2) * (n2 + m2 - 1);
                }
        ...
        public LipkinTwoSpectator(Core.Import import) : base(import) { }

        protected override void Export(IEParam param) {
            base.Export(param);

            param.Add(this.alphaS, "AlphaS");
            param.Add(this.omegaS, "OmegaS");
            param.Add(this.g, "g");
        }
        ...
        protected override void Import(IEParam param) {
            ...
            this.alphaS = (double)param.Get(0.0);
            this.omegaS = (double)param.Get(0.0);
